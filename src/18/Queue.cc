...
template <class Type>
void
Queue<Type>::copy(const Queue &orig)
{
...
Queue<Type>& Queue<Type>::operator=(const Queue &rhs)
{
    if (this != &rhs) {
        ...
        destroy();
        copy(rhs);    // copy from rhs into left-hand Queue
    }
    return *this;
}
...
void Queue<T>::assign(Iter beg, Iter end)
{
    ...
    copy(beg, end); // copy elements from the input range into this Queue
}

template <class Type> template <class Iter>
void Queue<Type>::copy(Iter beg, Iter end)
{
    while (beg != end) {
       push(*beg);  
       ++beg;
